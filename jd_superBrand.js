      ...
      if ($.cando) {
        ...
        if ($.actid && $.enpid) {
          ...
          await doTask(
            'secondfloor',
            $.enpid,
            $.taskList.encryptAssignmentId,
            $.taskList.ext.followShop[0].itemId,
            $.taskList.assignmentType
          );
        }
...
function doTask(source, pid, encryptAssignmentId, id, type) {
  return new Promise(async (resolve) => {
    const options = taskPostUrl(
      `superBrandDoTask`,
      `{"source":"${source}","activityId":${$.actid},"encryptProjectId":"${pid}","encryptAssignmentId":"${encryptAssignmentId}","assignmentType":${type},"itemId":"${id}","actionType":0}`
    );
    //    console.log(options)
    $.post(options, async (err, resp, data) => {
      try {
